...
for floor in rooms_by_floor.keys():
    ...
    for room in floor_rooms:
        ...
        report_lines.append(f"  地面: {room['地面面积']}㎡ | 墙面: {room['墙面面积']}㎡ | 顶面: {room['顶面面积']}㎡")
        for item in room["项目"]:
            report_lines.append(f"    [{item['类别']}] {item['名称']}: {item['数量']} {item['单位']}")

report_lines.append("\n" + "=" * 80)
report_lines.append("【设备类】")
report_lines.append("=" * 80)
for item in material_list["设备"]:
    report_lines.append(f"  {item['名称']}: {item['数量']} {item['单位']}")

report_lines.append("\n" + "=" * 80)
report_lines.append("【上下水类】")
report_lines.append("=" * 80)
for item in material_list["上下水"]:
    report_lines.append(f"  {item['名称']}: {item['数量']} {item['单位']}")

report_lines.append("\n" + "=" * 80)
report_lines.append("【强弱电类】")
report_lines.append("=" * 80)
for item in material_list["强弱电"]:
    report_lines.append(f"  {item['名称']}: {item['数量']} {item['单位']}")

report_lines.append("\n" + "=" * 80)
report_lines.append("【拆除砌筑类】")
report_lines.append("=" * 80)
for item in material_list["拆除砌筑"]:
    report_lines.append(f"  {item['名称']}: {item['数量']} {item['单位']}")

report_lines.append("\n" + "=" * 80)
report_lines.append("清单已保存至: 项目/灏园别墅_20260425/报价/灏园别墅主材清单.json")
report_lines.append("=" * 80)
...
# 保存报告到文件
with open("项目/灏园别墅_20260425/报价/灏园别墅主材清单.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(report_lines))

print(f"\n报告已保存至: 项目/灏园别墅_20260425/报价/灏园别墅主材清单.txt")
