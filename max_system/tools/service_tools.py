"""售后工具：竣工回访、问题处理、售后台账、老客户维护、客诉查询"""
...
def _get_after_sales_table_id() -> str:
    if _settings is None:
        raise RuntimeError("service_tools 未初始化")
...
async def complaintpro_handle(args: dict) -> dict:
    """查询客户的历史问题和项目数据，返回结构化信息供LLM制定处理方案"""
    client_name = args.get("client_name", "")
    project_name = args.get("project_name", "")
    ...
    # 查询该客户的历史售后问题
    related_issues = []
    for iid, issue in _afterlog_db.items():
        if project_name and project_name in issue.get("项目", ""):
            related_issues.append(issue)
        elif client_name and client_name in issue.get("项目", ""):
            related_issues.append(issue)

    # 处理时效框架
    deadline_map = {
        "重大投诉": "24小时内响应，48小时内书面方案",
        "严重投诉": "4小时内响应，3个工作日内方案",
        "一般投诉": "24小时内响应，7个工作日内整改",
    }

    result = {
        "客户信息": client_data or {"name": client_name, "note": "未在数据库中找到"},
        "投诉内容": content,
        "严重程度": severity,
        "处理时效要求": deadline_map.get(severity, "7个工作日内"),
        "历史售后问题": related_issues or "无历史记录",
        "note": "请根据以上信息制定客诉处理方案和沟通话术。",
    }
...
TOOL_DEFS = [
    ...
    },
    {
        ...
        "description": "查询客户信息和历史售后问题，用于制定客诉处理方案。输入客户姓名和投诉内容。",
        "parameters": {
            "type": "object",
            "properties": {
